...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(    
            ...
            actionButton("stats", "Get LM Model Stats"),
        ),
...
server <- function(input, output) {
    ...
    output$lmStats <- eventReactive(input$modelStats, {
        model = lm(formula = y ~ x, data = dataInput())
        model_summary <- summary(model())
    ...
    output$distPlot <- renderPlot({
        ggplot() +
            ...
            colour = 'green') + ggtitle('Values from Input Data') +
    ...
    output$lmPlot <- renderPlot({
        ggplot() +
            ...
            colour = 'blue') +
            ggtitle('Linear Modeled values from Input Data') +
            xlab('X Values in uploaded data file') +
            ylab('Y Values in uploaded data file')
    })
